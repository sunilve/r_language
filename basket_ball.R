...
accuracy<-round(FieldGoals/FieldGoalAttempts,2)
accuracy
class(accuracy)
accuracy<-t(accuracy)
matplot(accuracy,type = "b",pch=15:16,col=1:6)
legend("bottomleft",inset = .01,legend = Players,col = 1:6,pch=15:16,horiz = F)


Games
Players
#duration for which player is on ground per game
duration<-MinutesPlayed/Games
duration["DerrickRose","2012"]<-32.8
duration
max(duration)
d1<-t(round(duration,1))
d1[seq(1,5),c(1,6)]
x<-d1[,1]
x
max(x)
...
mode(x)
sd(x)
rnorm(x,mean = mean(x),sd=sd(x))
max(duration[3:7,1:5])

#function
mat<-function(data,row=1:10){
  Data<-data[row,,drop=FALSE]
  matplot(t(Data),type="b",pch=15:18,col = c(1:4,6))
  
  legend("bottomleft",inset = .01,legend=Players[row],col = c(1:4,6),pch=15:18,horiz = F)    
  
}
mat(Salary)
mat(Salary/Games)
mat(Salary/FieldGoals)
mat(FieldGoals/FieldGoalAttempts)
...
Freethrow<-rbind(KobeBryant_FT,JoeJohnson_FT,LeBronJames_FT,CarmeloAnthony_FT,
...
rm(KobeBryant_FT,JoeJohnson_FT,LeBronJames_FT,CarmeloAnthony_FT,
   DwightHoward_FT,ChrisBosh_FT,ChrisPaul_FT,KevinDurant_FT,DerrickRose_FT,DwayneWade_FT)
colnames(Freethrow)<-Seasons
rownames(Freethrow)<-Players
Freethrow
...
Freethrow_attempt<-rbind(KobeBryant_FTA,JoeJohnson_FTA,LeBronJames_FTA,CarmeloAnthony_FTA,DwightHoward_FTA,
                         ChrisBosh_FTA,ChrisPaul_FTA,KevinDurant_FTA,DerrickRose_FTA,DwayneWade_FTA)
rownames(Freethrow_attempt)<-Players
colnames(Freethrow_attempt)<-Seasons
Freethrow_attempt


# Solution 1

x<-Freethrow_attempt/Games
matplot(t(x),type = "b",pch = 15:18,col = 1:6)
legend("bottomleft",inset=.01,legend = Players,pch = 5:6,col = 1:6,horiz = F)

test<-function(x){
  matplot(t(x),type = "b",pch = 15:18,col = 1:6)
  legend("topright",inset=.01,legend = Players,pch = 5:6,col = 1:6,horiz = F)
  
  
}
# use x matrix as your required matrix for question 1,2&3

x<-(Points-Freethrow)/FieldGoals
test(x)
acccuracy<-Freethrow/Freethrow_attempt

test(x)

#solution2
test_2<-function(matr){
  
matplot(t(matr),type="b",pch = 15:18,col=1:6)
legend("topright",inset = .01,legend = Players,pch = 15:18,col = 1:6,pch = 5:6,horiz = F)
}
test_2(accuracy)
